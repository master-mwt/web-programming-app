        ...
        @foreach($posts as $post)
            <div class="card col-lg-10 mx-auto d-flex flex-row px-0" style="max-width: 800px">
                ...
                <div class="col p-0 d-flex flex-column overflow-auto">
                    ...
                    <div class="card-body text-left px-3 py-1 mb-1">
                        <div class="collapse" id="content-collapse-{{$post->post_id->id}}">
                            <div class="markdown-content" data-markdown-content="{{ $post->post_id->content }}"></div>
                        </div>
                        @foreach($post->tags as $tag)
                            @if($tag->tag_id->name == $query)
                            <span class="badge badge-pill mt-2" style="font-size: 11px; background-color: #ffa07a">{{$tag->tag_id->name}}</span>
                            @else
                            <span class="badge badge-pill mt-2" style="font-size: 11px; background-color: #ddd">{{$tag->tag_id->name}}</span>
                            @endif
                        @endforeach
                    </div>
